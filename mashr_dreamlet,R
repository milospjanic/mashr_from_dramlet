#### MASHR STARTING FROM DREAMLET OUTPUT ####
...
run_mash <- function(dreamlet_output) {

tabulator<-read.csv(dreamlet_output)

  # compute standard error from t-stat and logFC
  tabulator$se <- tabulator$logFC / tabulator$t
...
res_mash = run_mash(dreamlet_output='/sc/arion/projects/CommonMind/mpjanic/PEC/dreamlet_1.3.3/final/mdd.csv')
...
write.csv(mashr_output, "mdd_mashr_post.csv", quote=FALSE)
write.csv(mashr_pval, "mdd_mashr_pval.csv", quote=FALSE)

prob <- compositePosteriorTest(res_mash, colnames(mashr_output))
write.csv(prob, "mdd_mashr_comppost.csv", quote=FALSE)
